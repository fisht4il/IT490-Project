...
function doValidate($sessionId) {
    try {
		...
		$userId = doGetId($sessionId, $pdo);

		//this is only for debug test
		if (!$userId){
			$userId = 3;
		}
		    ...
		    "user_id" => $userId,
		    "balance" => showTextBalance($userId)//doGetId($sessionId))
                ];
            }
        } else {
            return [
...
function doGetId($sessionId, $pdo){
	try{
		$stmt = $pdo->prepare("SELECT u.id FROM sessions s JOIN users u ON s.username = u.username WHERE s.id = :session");
		$stmt->bindParam(':session', $sessionId, PDO::PARAM_INT);
		$stmt->execute();
		$user = $stmtId->fetchColumn();
...
//rabbitmq required for frontend interaction. dogetbalance is rabbitmq, this just updates based on what is already returned
function showTextBalance($userId){
	$reply = doGetBalance($userId);
